...
def non_max_suppression(prediction, conf_thres=0.1, iou_thres=0.6, merge=False, classes=None, agnostic=False):
    ...
    for xi, x in enumerate(prediction):  # image index, image inference
        ...
        # x[:, 5:]对除了前五列之外的列进行操作，第六列是每个类别的置信度
        # x[:, 4:5]对第五列和第六列之间的数据进行操作，不包括第六列，对象置信度所在列
        # 将每个对象置信度和类别置信度相乘，得到综合置信度；同时使用广播机制确保，相乘两元素形状匹配
...
def output_to_target(output, width, height):
    ...
    for i, o in enumerate(output):
        if o is not None:
            # add this, sometimes output can be a list of tensor, so here ensure the type again, this fixes the error.
...
def plot_one_box(x, img, color=None, label=None, line_thickness=None):
    ...
    if label:
        ...
        # label 文本字符串  0表示字体类型为默认值
        c2 = c1[0] + t_size[0], c1[1] - t_size[1] - 3
        # 修改c2为文本框的右下角坐标
        cv2.rectangle(img, c1, c2, color, -1, cv2.LINE_AA)  # filled
        cv2.putText(img, label, (c1[0], c1[1] - 2), 0, tl / 3, [225, 255, 255], thickness=tf, lineType=cv2.LINE_AA)
        # (c1[0], c1[1] - 2)略微向上偏移，文本不会和文本框边缘重叠；0默认字体；tl/3字体缩放因子；[225, 255, 255]文本颜色是白色
